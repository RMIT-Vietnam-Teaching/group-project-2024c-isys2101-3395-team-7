        ...
        <>
            <div className="mt-6 bg-pink text-white rounded-lg shadow-md">
                ...
                {filledData.map((item, index) => (
                    <div
                        key={index}
                        className={`grid grid-cols-5 p-4 border-t border-pink ${item.answer ? 'bg-black hover:bg-orange hover:cursor-pointer hover:text-black' : 'bg-transparent'}`}
                        onClick={item.answer ? () => {
                    ...
                    >
                        <div>{item.answer || '\u00A0'}</div>
                        <div>{item.transcribedText || (item.answer ? "Raw text" : '\u00A0')}</div>
                        <div>{item.time || '\u00A0'}</div>
                        <div>{item.type || '\u00A0'}</div>
            ...
            <div className="flex justify-center mt-4">
                {Array.from({ length: totalPages }, (_, index) => (
                    <button
                        ...
                        className={`px-3 py-1 mx-1 rounded ${currentPage === index + 1 ? 'bg-pink text-white p-3' : 'bg-[#D9D9D9] text-black'}`}
                    >
                        {index + 1}
                    </button>
                ))}
